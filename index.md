...
Polygon 的版本管理机制和 git 类似，**以 commit 作为管理改动的粒度**。你可以暂时切换到过去的某个版本（类似通过 detached head 的方式 checkout 到过去的某个提交），或者是通过 revert 回滚到过去的版本（会生成一个新的 commit，内容为这些改动的回滚）。

如果你用过 git，这些内容对你而言应该会很熟悉。

虽然部分细节上（尤其是遇到冲突的时候）不一定足够智能，但是也足够团队出题用了。

### 版本管理与 commit

你对题目进行编辑之前，Polygon 会先创建一个工作副本（working copy），你可以理解为一个暂存区。你所有的修改都将基于该工作副本进行，并且随时可以把它删掉。

页面右下角，我们可以看到目前有哪些文件是被我们修改过的。
...
在 `View Chagnes` 页面，点击某个版本旁边的 `switch`，可以**暂时**切换到某个过去的版本（回到 View Problems 页面再重新进入题目，将会重新回到最新版本）。

### 回滚到过去某个版本

假设当前已有 $$N$$ 个版本。当你点击 `revert` 时，可将当前工作副本回退到某个历史版本 $$K$$。你可以在该版本基础上进行修改，commit 后会产生新版本 $$N + 1$$。版本 $$K + 1$$ 到 $$N$$ 之间的所有历史记录均会保留，不会被覆盖掉。

举个例子，回滚前题目的 commit 形如：

- 版本 1：创建题目；
- 版本 2：增加测试数据 std 与 checker 等信息，题目基本完成；
- 版本 3：将 $n$ 调整为 $10^4$，$T$ 调整为 $3$，增加题目的卡常属性；
- 版本 4：在题面里增加了一段意义不明的题目背景；
- 版本 5：发现题目有 $\frac{n^2}{w}$ 的做法，将 $n$ 调整为 $10^5$；

这个时候，你想要回退到版本 3，于是对版本 3 点击了 revert。随后，你的 working copy 变成了版本 3 的状态。提交所有的改动。这时候，题目的 commit 变为：

- 版本 1：创建题目；
- 版本 2：增加测试数据 std 与 checker 等信息，题目基本完成；
- 版本 3：将 $n$ 调整为 $10^4$，$T$ 调整为 $3$；
- 版本 4：在题面里增加了一段意义不明的题目背景；
- 版本 5：发现题目有 $\frac{n^2}{w}$ 的做法，将 $n$ 调整为 $10^5$；
- 版本 6：回退了「将 $n$ 调整为 $10^4$，$T$ 调整为 $3$」「在题面里增加了一段意义不明的题目背景」「发现题目有 $\frac{n^2}{w}$ 的做法，将 $n$ 调整为 $10^5$」这几项改动。
...
## Solution files：管理所有的正解与错解 

在 Solution files 页面，你可以上传题目的相关代码。

第一个上传的代码会被标记为 Main correct solution，即最核心的 std 代码。当然，标记之后是可以改的。
...
在 Tests 页面，我们可以添加/管理题目的测试数据。

### 手动添加测试数据

我们可以先手造一些小数据，点击 `Add Test` 进行添加。
...
对于多组测试用例的情况，可以使用 `setTestCase(XXX);` 来告诉 testlib，我现在在检验第几组测试用例。这样做有诸多好处。

第一个好处是，当你将题目导入 Codeforces 平台后，在预览样例的界面能够看到特殊的高亮，让展示效果更加清晰好看。详见 [mike 发的博客](https://codeforces.com/blog/entry/146521)。

![](img/cf_multitest_example.png)
...
## Checker：输入是 a，输出是 b，答案是 c。输出对吗？

如果你不明白 Checker 这个东西为什么会被这样设计，不妨仔细思考一下这个问题。

对于答案是确定的题（比如输入 $x$ 和 $y$，输出 $x + y$），直接比较选手程序的输出与 std 的输出是否完全相等，固然是最简单的一种方式。

可是随之而来的也有很多问题，比方说选手程序多输出了个空格，或者是少输出了个换行，就会被判定为答案错误。这显然太丧心病狂了。

除此之外，面对“存在多种可行的答案，可以输出其中任意一种”类型的题，比较是否完全相等就更不可行了。

因此，我们需要借助 Checker 来进行输出正确性的判断。

Checker 是一个程序，它负责根据**输入数据、std 输出，选手程序输出**这三项内容，判断选手程序是否输出正确。

### 默认 Checker

Polygon 为我们提供了很多许多人性化的 Checker。
...
### 自定义 checker 记得使用 setTestCase

具体原因跟上面 Validator 章节讲述的一样。这里不再重复。

### 我要设计没卵用的样例，防止选手猜出 std 的构造

前面提到过，Polygon 的设计是，你只需要设计测试数据的输入部分，而测试数据的输出部分是通过 std 自动跑出来的。包括样例也是。

你设置了样例的输入之后，Polygon 会自动帮你跑出样例的输出，放在题面中。

但某些时候，你可能很不希望选手直接看到 std 的输出。比如在某些构造题里，选手可能通过 std 的输出，就能直接猜到 std 使用的构造算法。

这种情况下，你可能需要手动更改一下样例的输出。

具体的，我们有两种简单的可行的方式。

第一种方式是，**新增**一份 std，并将其设为 Main Correct Solution。在代码中，特判一下样例：当遇到样例的时候，输出你准备好的没卵用构造。

这种做法下，请务必保留原有的 std。这样一来，你的特判写出问题的时候，还可以因为两份 std 在样例上表现不一致（比如一个输出无解一个输出有解）被 Polygon 检查出问题。
...
第二种方式有可能导致题目导入 cf 之后样例的高亮出现问题（比如自定义输出未被高亮）。

如果你很在乎这个高亮，可以考虑使用第一种方式。

![](img/cf_multitest_example_2.png)
...
## Package 以及 Verification

在 Package 页面，你可以将题目的所有关键内容（题面、样例、数据、Validator、Checker 等）进行打包。**在打包之前，记得先 commit 所有修改**。

![](img/create_package.png)

Package 分为 Standard Package、Linux Package、Windows Package 三种，这些包的区别将会在后面进行介绍。

点击 `Standard` 可以构建一个 Standard Package。点击 `Full` 可以同时构建上述三种 Package。

勾选上 `Verify` 的话，Polygon 会附带帮你跑一次 Verification。我们下面会简单讲解一下 Verification 的作用。**请保证它被勾上**。别取消。

### Verification 与警告信息

Verification 通常能帮你检查出很多题目中存在的问题或隐患。

比如所有标记为 Correct solution 的代码是否全都表现为正确，标记为 Time limit exceeded 是否都能在部分数据上 TLE。

再比如你没有造放满数据（具体的，就是 $$n$$ 没有达到上下界、$$a_i$$ 没有达到上下界，诸如此类），那么你能够在此看到 `XXX didn't hit maximal/minimal` 的警告。
...
或者也可以直接使用 [这个仓库](https://github.com/fy-hb/polygon-chinese-template) 中的模板文件。相比于原版做出了更多的修改，具体见 README 部分。
...
[olymp.sty](https://github.com/GassaFM/olymp.sty) 是一个样式包，其中附带了很多方便写编写一份题面的样式与环境。
...
在 Polygon 的 Edit with Preview 功能中，你可以正常使用 ```\lt``` 表示小于号（或者 ```\gt``` 表示大于号）。但是它其实是 MathJax（也就是网页上使用的 LaTeX 数学公式渲染引擎）扩展，而非标准 LaTeX 语法。如有需要，请直接使用小于号符号（或大于号符号）本身。

### 预览测试时遇到 ERROR: RepositoryException

通常由于没有上传 std 或者没有设置/上传 checker 导致。

通常右侧的 Answer 栏会告诉你具体原因（比如 `Checker not found.` 或者是 `Can't find model (main correct) solution`）。

![](img/repo_error_no_checker.png)

![](img/repo_error_no_std.png)

### std::ncmp.cpp 报错显示 1st numbers differ - expected: '0', found: '0'

将 [testlib](https://github.com/MikeMirzayanov/testlib) 升级到最新版本并重新编译 checker 即可解决。具体原因详见[这一 issue](https://github.com/MikeMirzayanov/testlib/issues/222)。以下简单概括一下。

出现这一报错意味着：选手答案和 std 输出**确实不一致**，只是 std::ncmp.cpp 在输出错误信息时出现了问题。

具体的，`std::ncmp.cpp` 使用 testlib 提供的 `englishEnding` 以及 `vtos` 这两个函数格式化输出信息。
...
这两个函数返回的是 `std::string`，但是被 mike 错误的加上了 `__attribute__((const))`（这是一个 gcc 拓展，用于声明这是一个无任何副作用的纯函数，方便编译器优化）。

结果就是，在 gcc 15 及以上，优化等级为 O2 及以上的时候，`vtos` 这个函数会被编译器优化为始终返回 $0$，于是便导致了这一问题。
